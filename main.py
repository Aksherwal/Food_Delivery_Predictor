...
def dashboard():
    if request.method == 'POST':
        ...
        cur.execute('select delivery_person_age from delivery_data where name=%s', (name,))
        age = cur.fetchone()[0]
        cur.execute('select delivery_person_ratings from delivery_data where name=%s', (name,))
        rating = cur.fetchone()[0]
        cur.execute('select type_of_vehicle from delivery_data where name=%s', (name,))
        vehicle = cur.fetchone()[0]
        cur.execute('select multiple_deliveries from delivery_data where name=%s', (name,))
